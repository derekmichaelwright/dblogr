...
d1 <- agData_FAO_LandUse %>% 
  filter(Item %in% myItems1, Measurement == "Area") %>%
  left_join(pp, by = c("Area","Year")) %>%
  mutate(Item = mv(Item, myItems1, myItems2),
         Item = factor(Item, levels = myItems2),
         AreaPerPerson = Value / Population)
d2 <- agData_FAO_LandUse %>% filter(Measurement == "Area")
#
myCaption <- "Data: FAOSTAT<br>derekmichaelwright.github.io/dblogr/agdata/farmland_world"
```
...
# Plot 
mp1 <- ggplot(xx, aes(x = Year, y = Value / 1000000, color = Item)) +
  geom_line(size = 1.5, alpha = 0.8) +
  facet_wrap(Item ~ ., scales = "free_y") +
  scale_color_manual(values = myColors) +
  theme_agData(legend.position = "none") +
  labs(title = "A)", y = "Billion Hectares", x = NULL, caption = "")
mp2 <- ggplot(xx, aes(x = Year, y = Value / 1000000, fill = Item)) +
...
mp <- ggplot(xx, aes(x = Year, y = Value, fill = Item)) + 
  ...
  scale_y_continuous(breaks = 0:3 * 1000000, labels = 0:3, 
                     sec.axis = mySA) +
...
# Plot Data
mp <- ggplot(xx, aes(x = Area, y = Value / 1000, fill = Area)) + 
...
mp <- ggplot(xx, aes(x = Area, y = Value / 1000, fill = Region)) + 
  ...
  coord_flip(ylim = c(0.02, max(xx$Value) / 1000)) +
...
mp <- ggplot(xx, aes(x = Area, y = Value / 1000, fill = Region)) + 
  ...
  scale_x_discrete(limits = rev(levels(xx$Area))) +
  theme_agData_col(legend.position = "bottom", horizontal = T) +
  coord_flip(ylim = c(0.0009, max(xx$Value) / 1000)) +
...
ggAgRegion <- function(myItem = "Crop Land", myRegion = "Asia") {
  ...
  xx <- d1 %>% 
    ...
    mutate(Area = factor(Area, levels = unique(Area)))
  # Plot Data
  ggplot(xx, aes(x = Area, y = Value / 1000, fill = Region)) + 
...
ggAgRegion <- function(myRegion = "Americas", 
  ...
  xx <- d1 %>% 
    ...
    top_n(n = 10)
  # Plot Data
  ggplot(xx, aes(x = Area, y = Value / 1000)) + 
