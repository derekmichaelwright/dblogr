...
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/agdata/wheat_canada)"
---
...
```{r}
# Prep data
myCaption <- "Data: STATCAN<br>derekmichaelwright.github.io/dblogr/agdata/wheat_canada"
# 
d1 <- agData_STATCAN_Crops %>% filter(Item == "Wheat")
#
myCrops <- c("Wheat, durum", "Wheat, spring", 
...
```{r}
# Prep data
myColors <- c("darkorange", "darkgreen", "steelblue", "darkred")
...
xx <- d2 %>% 
  ...
  mutate(Measurement = mv(Measurement, myMeasures,
...
xx <- agData_STATCAN_Crops2 %>% 
  ...
  mutate(Item = mv(Item, myCrops[3:4], rep(myCrops[2],2)),
         Measurement = mv(Measurement, myMeasures,
                          c("Production (Tonnes)", "Area Seeded (Hectares)")))
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, color = Measurement)) +
  geom_line(size = 1, alpha = 0.7) +
...
## spring vs winter 1920s 

![](wheat_canada_07.png)

```{r}
# Prep data
myMeasures <- c("Area Seeded")
myCrops <- c("Wheat, spring", "Wheat, winter",
             "Wheat, winter seeded in fall", "Wheat, winter remaining")
xx <- agData_STATCAN_Crops2 %>% 
  filter(Area == "Ontario", 
