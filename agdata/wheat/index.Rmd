...
```{r}
# Prep data
myCaption <- "Data: FAO<br>derekmichaelwright.github.io/dblogr/agdata/wheat"
#
...
xx <- dd %>%
  mutate(Value = ifelse(Measurement %in% c("Area Harvested", "Production"),
         ...
         Unit = mv(Unit, c("Hectares", "Tonnes", "kg/ha"), 
...
```{r}
# Prep data
myCs <- c("darkred", "steelblue", "darkgreen", "darkblue", "darkorange")
myAs <- c("Africa", "Oceania", "Americas", "Europe", "Asia" )
xx <- dd %>% 
  filter(Area %in% myAs, 
...
mp <- ggplot(xx, aes(x = "", y = Prop, fill = Area)) +
  ...
  geom_label(aes(y = ypos, label = Label), nudge_x = 0.1,
             fill = "white", alpha = 0.8, lwd = 0, size = 3) +
  scale_fill_manual(name = NULL, values = myCs) +
  guides(fill = guide_legend(reverse = T)) +
  theme_agData_pie(caption.adj = -0.4) +
  labs(title = "Global Wheat Production 2019", caption = myCaption) 
ggsave("wheat_01.png", mp, width = 4, height = 3)
```

```{r echo = F}
ggsave("featured.png", mp, width = 4, height = 3)
```
...
```{r}
# Prep data
myCs <- c("darkgreen", "darkblue", "darkred", "darkorange", "steelblue")
xx <- dd %>% 
...
mp <- ggplot(xx, aes(x = Area, y = Value / 1000000, fill = Region)) +
  ...
  theme_agData_col(legend.position = "none",
                   axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Global Wheat Production - 2019", x = NULL,
...
```{r}
# Prep data
myCs <- c("darkgreen", "darkred", "steelblue", "darkorange")
myAs <- c(c("Rest of World", "Russia", "Ukraine", "USSR"))
xx <- dd %>% 
  filter(Measurement == "Production",
         Area %in% agData_FAO_Country_Table$Country)
x1 <- xx %>% filter(Area %in% myAs)
...
mp <- ggplot(x1, aes(x = "", y = Prop, fill = Area)) +
  geom_col(alpha = 0.7) +
  coord_polar("y", start = 0) +
  geom_label(aes(y = ypos, label = paste0(Prop,"%")), nudge_x = 0.1,
             fill = "white", alpha = 0.8, lwd = 0, size = 3) +
  scale_fill_manual(name = NULL, values = myCs) +
  guides(fill = guide_legend(reverse = T)) +
  theme_agData_pie(caption.adj = -0.5) +
  labs(title = "Global Wheat Production - 2019", caption = myCaption) 
ggsave("wheat_04.png", mp, width = 4, height = 3)
```
...
mp <- ggplot(x1, aes(x = "", y = Prop, fill = Area)) +
  ...
  guides(fill = guide_legend(reverse = T)) +
  theme_agData_pie(caption.adj = -0.5) +
  labs(title = "Global Wheat Production - 1965", caption = myCaption) 
ggsave("wheat_05.png", mp, width = 4, height = 3)
```
