...
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/agdata/crops_saskatchewan)"
---
...
```{r}
myCaption <- "Data: STATCAN<br>derekmichaelwright.github.io/dblogr/agdata/crops_saskatchewan"
#
...
xx <- bind_rows(x1, x2) %>%
  ...
  mutate(Item = factor(Item, levels = rev(myCrops)),
         Group = mv(Year, myYears, myGroups))
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, fill = Item)) + 
...
x2 <- d1 %>% 
  ...
  mutate(Item = mv(Item, c("Canola", "Wheat"), 
                   c("Evil GMO Oilseeds (Canola)", "Toxic Gluten (Wheat)")))
xx <- bind_rows(x1, x2) %>% filter(Year < 2021) %>%
  mutate(Item = factor(Item, levels = c("Toxic Gluten (Wheat)", 
                                        "Fake Meat Proteins (Legumes)", 
