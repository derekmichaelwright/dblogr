...
```{r echo = F}
downloadthis::download_link(
  link = "https://dblogr.com/agdata/crops_saskatchewan/figures_crops_saskatchewan_statcan.pdf",
  button_label = "figures_crops_saskatchewan_statcan.pdf",
  button_type = "success",
  has_icon = TRUE,
  icon = "fa fa-file-pdf",
...
# Plot
pdf("figures_crops_saskatchewan_statcan.pdf", width = 12, height = 4)
for(i in myCrops) {
  xi <- xx %>% filter(Crop == i)
...
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, fill = Crop)) + 
  ...
  labs(title = "Saskatchewan - Drought Years", 
       y = "Million Tonnes", x = NULL,
       caption = "\u00A9 www.dblogr.com/  |  Data: STATCAN")
...
mp <- ggplot(xx, aes(area = Value, fill = Crop, label = Crop)) +
  ...
  scale_fill_manual(values = agData_Colors) +
  theme_agData(legend.position = "none") +
  labs(title = "Saskatchewan Cropland", subtitle = "Area Seeded 2020",
       caption = "\u00A9 www.dblogr.com/  |  Data: STATCAN")
