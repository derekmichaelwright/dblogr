...
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, 
                     color = Crop, alpha = Crop, size = Crop)) + 
  geom_line() +
...
```{r}
# Prep data
myCrops <- cropList(measurement = "Area Seeded", years = c(1970, 2020))
xx <- agData_STATCAN_Crops %>% 
  filter(Area == "Saskatchewan", Year %in% c(1970,2020),
         Measurement == "Area Seeded") %>%
  arrange(desc(Value)) %>%
  mutate(Crop = factor(Crop, levels = unique(.$Crop)))
# Plot
mp <- ggplot(xx, aes(x = "", y = Value / 1000000, fill = Crop)) + 
...
x1 <- agData_STATCAN_Crops %>% 
  ...
  mutate(Crop = "Fake Meat Proteins (Legumes)")
x2 <- agData_STATCAN_Crops %>% 
  filter(Area == "Saskatchewan", Measurement == "Production",
...
xx <- bind_rows(x1, x2) %>% filter(Year < 2021) %>%
  mutate(Crop = factor(Crop, levels = c("Toxic Gluten (Wheat)", "Fake Meat Proteins (Legumes)", 
