...
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/agdata/global_food_security)"
---
...
```{r}
# Prep Data
myCaption <- "Data: FAOSTAT<br>derekmichaelwright.github.io/dblogr/agdata/global_food_security"
...
dd <- left_join(dd, pp, by = c("Area", "Year")) %>%
  ...
  mutate(TonnesPerPerson = Production / Population,
         Item = mv(Item, myItems1, myItems2[-1]),
         Area = mv(Area, "Australia and New Zealand", "Au and NZ"),
...
mp <- ggplot(xx, aes(x = Cereals, y = Pulses)) + 
  ...
  geom_text_repel(aes(label = Area), size = 2, alpha = 0.8) +
...
xx <- bind_rows(agData_FAO_Trade_Import_Quantity,
...
```{r}
# Prep data
myPulses <- c("Lentils, dry", "Chick peas, dry", 
              "Peas, dry", "Peas, green", "Pigeon peas, dry", "Cow peas, dry",
              "Beans, dry", "Other beans, green", "Broad beans and horse beans, dry", 
              "Broad beans and horse beans, green")
xx <- agData_FAO_Crops %>%
  filter(Area == "World", Measurement == "Production",
         Item %in% myPulses) %>%
  mutate(Item = factor(Item, levels = myPulses))
#
mp <- ggplot(xx, aes(x = Year, y = Value/1000000)) +
  geom_line(color = "darkgreen", linewidth = 1) +
  facet_wrap(Item ~ ., ncol = 5) +
  theme_agData() +
  labs(title = "Pulses", y = "Million Tonnes", caption = myCaption)
ggsave("global_food_security_12.png", mp, width = 8, height = 5)
```
...
mp <- ggplot(xx, aes(x = Year, y = Value/1000000)) +
  ...
  theme_agData() +
  labs(title = "Pulses", y = "Million Tonnes", caption = myCaption)
ggsave("global_food_security_13.png", mp, width = 8, height = 5)
```
...
```{r}
# Prep data
xx <- xx %>% arrange(Area, Year) %>%
  group_by(Area) %>%
  mutate(GrowthRate = 100 * (Value - lag(Value)) / lag(Value))
#
mp <- ggplot(xx, aes(x = Year, y = GrowthRate)) +
  geom_line(color = "darkgreen", linewidth = 1) +
  facet_wrap(Item ~ ., ncol = 5, scales = "free_y") +
  theme_agData() +
  labs(title = "Pulses", y = "Growth Rate (%)", caption = myCaption)
ggsave("global_food_security_14.png", mp, width = 8, height = 5)
```
