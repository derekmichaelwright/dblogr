...
`r shiny::icon("save")` [https://github.com/derekmichaelwright/agData/raw/master/Data/agData_FAO_Trade_Quantity.csv.gz
...
myItems1 <- c("Oilcrops, Oil Equivalent", "Treenuts",
             "Vegetables Primary", "Fruit Primary", "Sugar Raw Centrifugal", 
...
dd <- agData_FAO_Crops2 %>% select(-Unit) %>%
  filter(Area %in% myAreas, Crop %in% myItems1) %>%
...
dd <- left_join(dd, pp, by = c("Area", "Year")) %>%
  ...
  mutate(TonnesPerPerson = Production / Population,
         Crop = plyr::mapvalues(Crop, myItems1, myItems2[-1]),
...
for(i in unique(dd$Crop)) {
  for(j in unique(dd$Area)) {
    # Population
    p1961 <- dd %>% 
      filter(Area == j, Crop == i, Year == 1961) %>% pull(Population)
    dd <- dd %>% mutate(Population_R1961 = ifelse(Area == j & Crop == i, 
              100*(Population - p1961) / p1961, Population_R1961))
    # Production
    p1961 <- dd %>% 
      filter(Area == j, Crop == i, Year == 1961) %>% pull(Production)
    dd <- dd %>% mutate(Production_R1961 = ifelse(Area == j & Crop == i, 
              100*(Production - p1961) / p1961, Production_R1961))
    # Area
    p1961 <- dd %>% 
      filter(Area == j, Crop == i, Year == 1961) %>% pull(`Area Harvested`)
    dd <- dd %>% mutate(Area_R1961 = ifelse(Area == j & Crop == i, 
              100*(`Area Harvested` - p1961) / p1961, Area_R1961))
    # Yield
    p1961 <- dd %>% 
      filter(Area == j, Crop == i, Year == 1961) %>% pull(Yield)
    dd <- dd %>% mutate(Yield_R1961 = ifelse(Area == j & Crop == i, 
...
mp <- ggplot(xx, aes(x = Year)) +
  ...
  geom_line(aes(y = Yield_R1961, color = "Yield"),
            size = 1.25, alpha = 0.7) +
  facet_wrap(. ~ Crop, ncol = 4, scales = "free_y") +
  scale_x_continuous(breaks = seq(1960, 2020, by = 10)) +
...
```{r}
# Prep data
xx <- dd %>% filter(Area == "World", Crop == "Cereals")
# Plot
mp <- ggplot(xx, aes(x = Year)) +
  geom_line(aes(y = Population_R1961, color = "Population"),
            size = 1.5, alpha = 0.7) +
...
```{r}
# Prep data
xx <- dd %>% filter(Area == "World", Crop %in% c("Cereals", "Pulses"))
# Plot
mp <- ggplot(xx, aes(x = Year)) +
  geom_line(aes(y = Population_R1961, color = "Population"),
            size = 1.5, alpha = 0.7) +
...
mp <- ggplot(xx, aes(x = Area, y = TonnesPerPerson, fill = Area)) + 
  ...
  facet_wrap(Crop ~ ., scales = "free", ncol = 7) +
...
ggsave("global_food_security_05.png", mp, width = 12, height = 4)
```
...
ggsave("global_food_security_08.png", mp, width = 10, height = 8)
```
...
mp <- ggplot(xx, aes(x = Year, y = TonnesPerPerson, color = Crop)) + 
  ...
  labs(title = "Global Food Security - SubRegions", x = NULL,
       y = "Tonnes Per Person", caption = myCaption)
ggsave("global_food_security_09.png", mp, width = 12, height = 8)
```
...
```{r}
# Prep data
myItems <- c("Wheat", "Maize", "Rice", "Sorghum")
...
```{r}
# Prep data
myItems <- c("Soybeans", "Beans, dry", "Chick peas", "Lentils")
xx <- agData_FAO_Trade_Quantity %>%
  filter(Item %in% myItems, Area %in% myRegions)# %>%
  #mutate(Area = factor(Area, levels = myAreas))
# Plot
