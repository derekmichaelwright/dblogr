...
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/agdata/lentil)"
---
...
```{r}
# Prep data
myCaption <- "Data: FAOSTAT<br>derekmichaelwright.github.io/dblogr/agdata/lentil"
#
d1 <- agData_FAO_Crops %>% filter(Item == "Lentils, dry")
d2 <- bind_rows(agData_FAO_Trade_Import_Quantity,
...
```{r results="hide"}
# Prep data
myCs <- c("darkgreen", "darkred", "darkgoldenrod2")
myAs <- c("World",
...
for(i in myAs) {
  print(ggplot(xx %>% filter(Area == i)) +
    ...
    scale_color_manual(values = myCs) +
...
```{r}
# Prep data
myAs <- c("Canada", "Southern Asia", "USA", "Turkey", 
          "Europe", "Africa", "Oceania" )
myCs <- c("darkgreen", "darkgoldenrod2", "darkblue", "darkslategrey", 
...
```{r}
# Prep data
myCs <- c("darkgreen", "darkgoldenrod2", "steelblue", "antiquewhite4",
          "darkslategrey",  "darkred", "darkblue" )
myRs <- c("Americas", "Southern Asia", "Oceania", 
...
myCs <- c("darkgreen", "darkgoldenrod2", "darkblue", "darkslategrey", 
          "steelblue", "darkred", "antiquewhite4")
xx <- d1 %>% 
  filter(Area %in% myAs, Measurement == "Area Harvested") %>%
  mutate(Area = factor(Area, levels = myAs) ) 
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, color = Area)) + 
  geom_line(size = 1.5, alpha = 0.7) + 
...
```{r}
# Prep data
myCs <- c("darkgreen", "darkgoldenrod2", "steelblue", "darkblue", 
          "darkslategrey", "darkred")
myRs <- c("Americas", "Southern Asia", "Oceania", "Western Asia", 
...
```{r}
# Prep data
myAs <- c("Canada", "India", "Ethiopia", "Turkey")
myCs <- c("darkgreen", "darkgoldenrod2", "darkred", "darkslategrey")
xx <- d1 %>%
  filter(Area %in% myAs, Measurement == "Yield") %>%
  mutate(Area = factor(Area, levels = myAs))
# Plot
...
```{r}
# Prep data
myAs <- c("World", "Canada", "India", "Nepal", "Bangladesh")
myCs <- c("black", "darkgreen", "darkgoldenrod2", "darkred", "darkslategrey")
xx <- d1 %>%
  filter(Area %in% myAs, Measurement == "Yield") %>%
  mutate(Area = factor(Area, levels = myAs))
# Plot
...
ggPie_single <- function(crop = "Lentils, dry", measurement = "Production", 
                         year = 2019, cutoff = 8,
                         myCs = agData_Colors[c(1:10,41:50)]) {
  # Prep data
  ...
  # Plot
  ggplot(yy, aes(x = "")) +
    geom_col(aes(y = Percentage, fill = Area), alpha = 0.7) +
    coord_polar("y", start = 0) +
    geom_label(aes(label = paste(Percentage, "%"), y = Label_y), 
               size = 2.5, alpha = 0.9, nudge_x = 0.3, lwd = 0) + 
    facet_grid(. ~ Year) +
    theme_agData_pie(caption.adj = -0.5) +
...
ggPie_anim <- function(crop = "Lentils, dry", measurement = "Production", 
  ...
  myAs <- c(xx %>% arrange(desc(Percent)) %>%
    filter(Year == max(xx$Year), Percent > cutoff) %>% 
    pull(Area) %>% as.character(), "Other")
  x1 <- xx %>% filter(Area %in% myAs) %>% arrange(desc(Percent))
  x2 <- xx %>% filter(!Area %in% myAs) %>% 
    group_by(Item, Measurement, Unit, Year) %>%
  ...
  ggplot(xx, aes(x = 1, fill = Area)) +
    geom_col(aes(y = -Percent), position = "fill", alpha = 0.7) +
    coord_polar("y", start = 0) +
    theme_agData_pie(caption.adj = -0.5) +
    scale_fill_manual(values = agData_Colors) +
    xlim(0.545, 1.5) +
...
ggPie_multi <- function(crop = "Lentils, dry", measurement = "Production", 
  ...
  xx <- xx %>% group_by(Year) %>%
    ...
    arrange(Percentage)
  myAs <- unique(c("Rest of World", xx$Area))
  xx <- xx %>%
    mutate(Area = factor(Area, levels = rev(myAs))) %>%
    arrange(Year, Area) %>%
    group_by(Year) %>%
  ...
  # Plot
  ggplot(xx, aes(x = 1, fill = Area)) +
    geom_col(aes(y = -Percentage), alpha = 0.7) +
    coord_polar("y", start = 0) +
    facet_grid(. ~ Year) +
    theme_agData_pie(caption.adj = -0.5) +
...
```{r}
# Prep data
myAs <- c("Americas","Asia","Europe","Africa","Oceania")
myCs <- c("darkgreen","darkorange","darkblue","darkred","steelblue")
xx <- d2 %>%
  region_Info() %>%
...
```{r}
# Prep data
x1 <- d1 %>% filter(Area == "Canada", Measurement == "Production") %>%
  select(Year, Production=Value)
x2 <- d2 %>% filter(Area == "Canada", Measurement == "Export Quantity") %>%
  select(Year, Export=Value)
xx <- left_join(x1, x2, by = "Year") %>% filter(!is.na(Export)) %>%
  gather(Measurement, Value, 2:3) %>%
  mutate(Measurement = factor(Measurement, levels = c("Production", "Export")))
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, fill = Measurement)) +
  geom_col(position = "dodge", alpha = 0.7) +
  scale_fill_manual(name = NULL, values = c("darkgreen","darkred")) +
  theme_agData() +
  labs(title = "", x = NULL, y = "Million Tonnes", caption = myCaption)
