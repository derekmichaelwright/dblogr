...
# Prep data
xx <- agData_FAO_Crops %>% 
  filter(Crop == "Lentils, dry", Measurement == "Production",
...
xx <- agData_FAO_Crops %>% 
  filter(Area %in% myAreas, Crop == "Lentils, dry", 
...
myRegions <- c("Americas", "Southern Asia", "Oceania", "Western Asia", 
               "Europe", "Central Asia")
x1 <- agData_FAO_Crops %>% 
  filter(Crop == "Lentils, dry", Measurement == "Area Harvested", Year == max(Year),
         Area %in% agData_FAO_Country_Table$Country) %>%
    top_n(10, Value)
xx <- agData_FAO_Crops %>% 
  filter(Crop == "Lentils, dry", Measurement == "Yield",
...
xx <- agData_FAO_Crops %>%
  filter(Crop == "Lentils, dry", Area %in% myAreas, Measurement == "Yield") %>%
  mutate(Area = factor(Area, levels = myAreas))
# Plot
...
```{r}
# Plot function
mp <- ggPie_anim(crop = "Lentils, dry", measurement = "Production", other = 2.5)
...
```{r}
# Plot function
mp <- ggPie_anim(crop = "Lentils, dry", measurement = "Area Harvested", other = 2.5)
...
xx <- agData_FAO_Trade_Quantity %>%
  filter(Item == "Lentils, dry", Area %in% c("Canada","India") )
# Plot
...
xx <- agData_FAO_Trade_Quantity %>%
  region_Info() %>%
  filter(Item == "Lentils, dry", Area %in% agData_FAO_Country_Table$Region) %>%
  mutate(Area = factor(Area, levels = myAreas))
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, 
...
# Prep data
x1 <- agData_FAO_Crops %>% 
  filter(Crop == "Lentils, dry", Measurement == "Production", Year == 2018,
         Area %in% agData_FAO_Country_Table$Country) %>%
    top_n(10, Value)
xx <- agData_FAO_Crops %>% 
  filter(Crop == "Lentils, dry", Measurement == "Production", 
...
x1 <- agData_STATCAN_Crops %>%
  filter(Crop == "Lentils, dry", Area == "Canada") %>%
