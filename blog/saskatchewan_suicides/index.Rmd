...
```{r}
# Prep data
myCaption <- "Data: SK GOV<br>derekmichaelwright.github.io/dblogr/blog/saskatchewan_suicides"
...
mp <- ggplot(d1, aes(x = Year, y = Deaths, fill = Sex)) +
  ...
  facet_grid(. ~ Age) +
  scale_x_continuous(breaks = c(2010, 2020), minor_breaks = 2005:max(d1$Year)) +
  scale_fill_manual(name = NULL, values = myColors) +
...
mp <- ggplot(d2, aes(x = Year, y = Deaths, fill = Sex)) +
  geom_bar(stat = "identity", color = "black", lwd = 0.2, alpha = 0.7) +
  facet_grid(. ~ Race) +
  scale_x_continuous(breaks = c(2010, 2020), minor_breaks = 2005:max(d2$Year)) +
  scale_fill_manual(name = NULL, values = myColors) +
  theme_agData(legend.position = "bottom") +
  labs(title = "Suicides in Saskatchewan", 
       x = NULL, caption = myCaption)
