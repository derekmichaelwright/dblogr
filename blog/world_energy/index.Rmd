...
xx <- d2 %>% filter(Area %in% myAreas) %>% 
  mutate(Area = factor(Area, levels = myAreas))
mp1 <- ggplot(xx %>% filter(Year >= 2000), 
              aes(x = Year, y = Value / 1000, color = Area)) +
  geom_line(alpha = 0.7) +
  facet_wrap(Area ~ ., scales = "free_y") +
  scale_color_manual(name = NULL, values = myColors) +
  theme_agData() +
  labs(title = "A) Propaganda For Canada", x = NULL,
       y = "Thousand kWh Per Person", caption = myCaption)
...
mp1 <- mp2 + facet_wrap(Area ~ ., scales = "free_y") +
  labs(title = "A)", caption = NULL)
mp <- ggarrange(mp1, mp2, ncol = 1, common.legend = T, legend = "bottom")
ggsave("world_energy_2_02.png", mp, width = 6, height = 5)
```
