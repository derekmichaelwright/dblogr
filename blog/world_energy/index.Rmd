...
```{r}
# Prep data
myCaption_OWID <- "Data: OurWorldInData<br>derekmichaelwright.github.io/dblogr/blog/world_energy"
myCaption_IEA <- "Data: IEA<br>derekmichaelwright.github.io/dblogr/blog/world_energy"
...
d3 <- read.csv("consumption-co2-per-capita-vs-gdppc.csv") %>%
  ...
  mutate(Country = mv(Country, oldnames, newnames)) %>%
...
xx <- xx %>%
  ...
  mutate(Group = paste(CO2diff, GDPdiff),
         Group = mv(Group, myGroups1, myGroups2),
...
xx <- d3 %>% select(Country, Year, GDP, CO2) %>%
    ...
    mutate(Measurement = mv(Measurement, c("CO2","GDP"),
