...
`r shiny::icon("globe")` https://www.cdc.gov/nchs/nvss/vsrr/covid19/excess_deaths.htm#dashboard

`r shiny::icon("globe")` https://www.census.gov/data/tables/time-series/demo/popest/2010s-state-total.html

`r shiny::icon("globe")` https://www.census.gov/topics/population/age-and-sex/data/tables.2021.List_897222059.html#list-tab-List_897222059

`r shiny::icon("globe")` https://www.census.gov/library/visualizations/interactive/how-has-our-nations-population-changed.html
...
ggYearlyDeathsAge <- function(myArea) {
  # Prep data
  xx <- dd %>% 
    filter(Type == "Unweighted", Area == myArea, Year < 2022) %>%
...
xx <- left_join(x1, x2, by = "Year") %>%
  mutate(Death.Rate = 10000 * Deaths / Population)
# Plot
...
ggsave("usa_deaths_7_01.png", mp, width = 6, height = 4)
```
...
x2 <- p2 %>% filter(Sex == "Both sexes", Age %in% myAges2[14:18]) %>%
  ...
  summarise(Population = sum(Population, na.rm = T))
xx <- left_join(x1, x2, by = "Year") %>%
  mutate(Death.Rate = 1000 * Deaths / Population)
# Plot
mp <- ggplot(xx, aes(x = Year, y = Death.Rate, fill = Group)) +
  geom_col(color = "black", alpha = 0.7) +
  scale_fill_manual(values = myColors) +
  theme_agData(legend.position = "none") +
  labs(title = "United States - Ages 65+", x = NULL,
       y = "Deaths Per 1,000 People", caption = myCaption2)
ggsave("usa_deaths_07_02.png", mp, width = 6, height = 4)
```
