...
myCs_Gender <- c("palevioletred3","steelblue")
...
d1 <- read.csv("1410028801_databaseLoadingData.csv") %>%
  select(Area=GEO, Date=REF_DATE, Measurement=Class.of.worker, 
         Gender, Unit=UOM, Scale=SCALAR_FACTOR, Value=VALUE) %>%
...
d2 <- d1 %>% 
  ...
  group_by(Area, Year, Measurement, Gender) %>%
  summarise(Value = sum(Value)) %>% ungroup() %>%
  group_by(Area, Year, Gender) %>% mutate(Total = sum(Value)) %>% ungroup() %>%
  mutate(Percent = 100 * Value / Total)
unique(d1$Measurement)
```
...
# Prep data
xx <- d1 %>% 
  filter(Area == "Canada", Gender == "Total - Gender", Measurement != "Total")
# Plot
...
gg_Priv_Gov <- function(myArea = "Canada") {
  # Prep data
  xx <- d1 %>% 
    filter(Gender == "Total - Gender", Date > "2010-01-01",
...
```{r}
# Prep data
xx <- d2 %>% filter(Area == "Canada", Gender == "Total - Gender", Measurement != "Total")
# Plot
...
xx <- d1 %>% 
  filter(Area == "Canada", Gender != "Total - Gender", Measurement != "Total")
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000000, color = Gender)) +
  geom_vline(xintercept = as.Date("2020-03-01"), lty = 2, alpha = 0.5) +
  geom_line(size = 1, alpha = 0.8) +
  scale_color_manual(name = NULL, values = myCs_Gender) +
...
xx <- d1 %>% 
  ...
  select(Date, Measurement, Gender, Value) %>%
  spread(Date, Value) %>%
  mutate(Change = `2022-05-15` - `2020-02-15`)
# Plot
mp <- ggplot(xx, aes(x = Gender, y = Change, fill = Gender)) +
  geom_col(position = "dodge", color = "black", alpha = 0.7) +
  facet_grid(. ~ Measurement) +
  scale_fill_manual(values = myCs_Gender) +
