...
```{r}
# Prep data
myCaption <- "Data: STATCAN<br>derekmichaelwright.github.io/dblogr/blog/canada_public_private"
...
xx <- d1 %>% 
  filter(Area == "Canada", Sex != "Both sexes", 
         Date %in% as.Date(c("2020-02-15","2025-05-15"))) %>%
  select(Date, Measurement, Sex, Value) %>%
  spread(Date, Value) %>%
  mutate(Change = `2025-05-15` - `2020-02-15`)
# Plot
mp <- ggplot(xx, aes(x = Sex, y = Change, fill = Sex)) +
  ...
  labs(title = "Change 2020-02-15 to 2025-05-15", x = NULL,
...
xx <- d1 %>% 
  filter(Area == "Canada", Sex != "Both sexes", 
         Date %in% as.Date(c("2019-09-15","2025-05-15"))) %>%
  select(Date, Measurement, Sex, Value) %>%
  spread(Date, Value) %>%
  mutate(Change = `2025-05-15` - `2019-09-15`)
# Plot
mp <- ggplot(xx, aes(x = Sex, y = Change, fill = Sex)) +
  geom_col(position = "dodge", color = "black", alpha = 0.7) +
  facet_grid(. ~ Measurement) +
  scale_fill_manual(values = myCs_Sex) +
  theme_agData_col(legend.position = "none") +
  labs(title = "Change 2019-09-15 to 2025-05-15", x = NULL,
       y = "Thousand", caption = myCaption)
