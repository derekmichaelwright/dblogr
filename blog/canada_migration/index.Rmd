...
GOV of CAN: Open Government - Monthly IRCC Updates - Source Countries - New Applications and Extensions Approved for Temporary Residents by Month

> - `r shiny::icon("globe")` [https://open.canada.ca/data/en/dataset/f82f66f2-a22b-4511-bccf-e1d74db39ae5](https://open.canada.ca/data/en/dataset/f82f66f2-a22b-4511-bccf-e1d74db39ae5){target="_blank"}
> - `r shiny::icon("save")` [Open Data - OPS TR Approved en.xlsx](Open Data - OPS TR Approved en.xlsx)
...
```{r}
# Prep data
myCaption <- "Data: STATCAN<br>derekmichaelwright.github.io/dblogr/blog/canada_migration"
...
d8 <- read_xlsx("Open Data - OPS TR Approved en.xlsx", range = "A7:AW223")
colnames(d8) <- substr(colnames(d8), 1, regexpr("\\.\\.\\.", colnames(d8))-1)
colnames(d8) <- c("Area", paste(colnames(d8)[2:ncol(d8)], rep(2022:2025, each = 13)))
d8 <- d8 %>%
  gather(Month, Value, 2:ncol(.)) %>%
  separate(Month, c("Month","Year"), sep = " ") %>%
  mutate(Value = as.numeric(Value),
         Area = mv(Area, "People's Republic of China", "China"))
d9 <- d8 %>% filter(Month != "Total") %>%
  mutate(MonthNum = mv(Month, myMonths, str_pad(1:12, width = 2, pad = "0")),
         Date = as.Date(paste(Year, MonthNum,"01", sep ="-"), format = "%Y-%m-%d"))
d8 <- d8 %>% filter(Month == "Total") %>% select(-Month)
```
...
# Plot
mp <- ggInterProv(myArea = "Ontario")
ggsave("canada_migration_3_06.png", mp, width = 20, height = 10)
```
...
# Plot
mp <- ggInterProv(myArea = "Ontario")
ggsave("canada_migration_3_07.png", mp, width = 20, height = 10)
```
...
mp <- ggplot(xx, aes(x = Age, y = Value, fill = Net)) +
  ...
  labs(title = "Alberta Net Interprovincial Migration", 
       y = "Thousand People", x = NULL, caption = myCaption)
ggsave("canada_migration_3_08.png", mp, width = 14, height = 6)
```
...
mp <- ggplot(xx, aes(x = Age, y = Value, fill = Net)) +
  ...
  labs(title = "British Columbia Net Interprovincial Migration", x = NULL,
       y = "Thousand People", caption = myCaption)
ggsave("canada_migration_3_09.png", mp, width = 8, height = 4)
```
...
```{r}
# Prep data
xx <- pp %>% arrange(Year) %>% mutate(Change = NA)
#
for(i in 1:nrow(xx)) {
  if(sum(xx$Year == xx$Year[i]-1 & xx$Area == xx$Area[i]) > 0) {
    xx$Change[i] <- xx$Population[i] - xx$Population[xx$Year == xx$Year[i]-1 & xx$Area == xx$Area[i]]
  }
}
xx <- xx %>% mutate(GrowthRate = 100 * Change / (Population-Change))
xx %>% filter(Area == "Canada")
# Plot
mp <- ggplot(xx, aes(x = Year, y = GrowthRate))  +
  geom_col() +
  facet_wrap(Area ~ .) +
  theme_agData()
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000000, fill = Type)) +
  ...
  labs(title = "Canada", subtitle = "Total, non-permanent residents", 
       y = "Million People", x = NULL, caption = myCaption)
ggsave("canada_migration_6_01.png", mp, width = 6, height = 5)
```
...
ggsave("canada_migration_6_02.png", mp, width = 14, height = 7)
```
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000, fill = Type)) +
  ...
  labs(title = "Canada - Temperary residents", 
       ...
       y = "Thousand People", x = NULL, caption = myCaption)
ggsave("canada_migration_6_03.png", mp, width = 6, height = 5)
```
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000, fill = Type)) +
  ...
  labs(title = "Canada - Temperary residents", 
       ...
       y = "Thousand People", caption = myCaption)
ggsave("canada_migration_6_04.png", mp, width = 14, height = 7)
```
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000, fill = Type)) +
  ...
  labs(title = "Canada - Asylum claimants, protected persons and related groups",
       y = "Thousand People", x = NULL, caption = myCaption)
ggsave("canada_migration_6_05.png", mp, width = 7, height = 5)
```
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000, fill = Type)) +
  ...
  labs(title = "Canada - Asylum claimants, protected persons and related groups",
       y = "Thousand People", x = NULL, caption = myCaption)
ggsave("canada_migration_6_06.png", mp, width = 14, height = 7)
```
...
![](canada_migration_6_07.png)

```{r}
# Prep Data
xx <- d5 %>% 
  filter(Area == "Canada", Type == "Total, non-permanent residents") %>%
  separate(Date, c("Year","Month"), sep = "-") %>%
  mutate(Year = as.numeric(Year)) %>%
  filter(Month == "01") %>%
  left_join(pp, by = c("Area","Year")) %>%
  mutate(Percent = 100 * Value / Population)
# Plot
...
ggsave("canada_migration_6_07.png", mp, width = 6, height = 4)
```

---

# Origin {.tabset .tabset-pills}

## 2024

![](canada_migration_7_01.png)

```{r}
# Prep data
xx <- d8 %>% 
  filter(Area != "Total", Year == 2024) %>%
  arrange(desc(Value)) %>%
  slice(1:10) %>%
  mutate(Area = factor(Area, levels = rev(Area)))
# Plot
mp <- ggplot(xx, aes(x = Area, y = Value / 1000)) +
  geom_col(fill = "steelblue", color = "black", alpha = 0.7) +
  theme_agData() +
  coord_flip() +
  labs(title = "2024 Origin of Temperary Residents", x = NULL, 
       y = "Thousand people", caption = myCaption)
ggsave("canada_migration_7_01.png", mp, width = 6, height = 4)
```
...
xx <- d8 %>% 
  ...
  mutate(Area = factor(Area, levels = rev(Area)))
# Plot
mp <- ggplot(xx, aes(x = Area, y = Value / 1000000)) +
  geom_col(fill = "steelblue", color = "black", alpha = 0.7) +
  theme_agData() +
  coord_flip() +
  labs(title = "Origin of Temperary Residents Since 2022", 
       x = NULL, y = "Million people", caption = myCaption)
ggsave("canada_migration_7_02.png", mp, width = 6, height = 4)
```
...
![](canada_migration_7_03.png)

```{r}
# Prep data
xx <- d9 %>% filter(Area %in% c("India","China","Ukraine")) 
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  geom_line(aes(color = Area), lwd = 1.5) +
  theme_agData() +
  scale_color_manual(values = c("darkred","goldenrod2","steelblue")) +
  labs(title = "Temperary Residents", x = NULL, 
       y = "Thousand people", caption = myCaption)
ggsave("canada_migration_7_03.png", mp, width = 6, height = 4)
```
...
![](canada_migration_7_04.png)

```{r}
# Prep data
xx <- d9 %>% filter(Area == "India") 
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  geom_col(fill = "goldenrod2", color = "black", alpha = 0.7) +
  theme_agData() +
  labs(title = "Temperary Residents From India", x = NULL, 
       y = "Thousand people", caption = myCaption)
ggsave("canada_migration_7_04.png", mp, width = 6, height = 4)
```
...
![](canada_migration_7_05.png)

```{r}
# Prep data
xx <- d9 %>% filter(Area == "China") 
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  geom_col(fill = "darkred", color = "black", alpha = 0.7) +
  theme_agData() +
  labs(title = "Temperary Residents From China", x = NULL, 
       y = "Thousand people", caption = myCaption)
ggsave("canada_migration_7_05.png", mp, width = 6, height = 4)
```
...
![](canada_migration_7_06.png)

```{r}
# Prep data
xx <- d9 %>% filter(Area == "Ukraine") 
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  geom_col(fill = "steelblue", color = "black", alpha = 0.7) +
  theme_agData() +
  labs(title = "Temperary Residents From Ukraine", x = NULL, 
       y = "Thousand people", caption = myCaption)
ggsave("canada_migration_7_06.png", mp, width = 6, height = 4)
```
...
mp
ggsave("canada_migration_6_01.png", mp, width = 6, height = 4)
#
xx <- d6 %>% 
  filter(Area == "Canada", Type == "Total - Non-permanent resident type",
...
mp <- ggplot(xx, aes(x = Age, y = Value / 1000, fill = Sex)) +
  ...
  labs(y = "Thousand People", caption = myCaption)
mp
ggsave("canada_migration_6_01.png", mp, width = 12, height = 4)
unique(d6$POB)
```
