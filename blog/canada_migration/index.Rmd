...
> - `r shiny::icon("globe")` https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=1710012101
> - `r shiny::icon("save")` [1710012101_databaseLoadingData.csv](1710012101_databaseLoadingData.csv)

STATCAN Table: 98-10-0361-01

> - `r shiny::icon("globe")` https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=9810036101
...
d4 <- read.csv("1710012101_databaseLoadingData.csv") %>%
  ...
  mutate(Area = gsub("Northwest Territories including", "NWT &", Area),
         ...
         Date = factor(Date))
#
myAges <- c("Total - Age", "0 to 14 years", "15 to 24 years",
            "25 to 64 years", "65 years and over")
d5 <- read.csv("9810036101_databaseLoadingData.csv") %>%
  select(Year=1, Area=GEO, Sex=Gender..3., Age=Age..15C., 
...
mp <- ggplot(xx, aes(x = Year, y = Value / 1000)) +
  geom_line(aes(color = Measurement), line.width = 1, alpha = 0.7) +
...
myW <- c("NonPrairies", "NonPrairies", "NonPrairies", 
...
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, fill = Measurement)) +
  ...
  theme_agData(legend.position = "bottom") +
  labs(title = "Canada", subtitle = "Immigrants & Non-Permanent Residents", 
...
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Area)) +
  geom_hline(yintercept = 0) +
  geom_line(line.width = 1.5, alpha = 0.7) +
...
```{r}
mp <- ggInterProv_ani("Saskatchewan")
...
# Prep Data
xx <- d4 %>% 
  filter(Type != "Total, non-permanent residents")
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000, fill = Type)) +
  ...
  theme_agData(legend.position = "bottom",
               axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Total, non-permanent residents", x = NULL,
       y = "Thousand People", caption = myCaption)
...
# Plot
mp <- ggplot(xx, aes(x = "", y = Value / 1000, fill = Sex)) +
  geom_col(alpha = 0.7) +
  facet_wrap(Age ~ .) +
