...
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/blog/saskatchewan_elections)"
---
...
```{r}
# Prep data
myCaption <- "Data: Elections SK<br>derekmichaelwright.github.io/dblogr/blog/saskatchewan_elections"
#
...
## Votes vs Seats {.tabset .tabset-pills}
...
mp <- ggplot(xx, aes(x = "", y = Percent)) +
  geom_col(aes(fill = Party), alpha = 0.7) +
...
## Rural Ridings {.tabset .tabset-pills}
...
## Urban Ridings {.tabset .tabset-pills}
...
myMeasures <- c("< 20% of votes", "< 30% of votes", 
                "< 40% of votes", "< 50% of votes",
                "> 50% of votes", "> 60% of votes",
                "> 70% of votes", "> 80% of votes")
x1 <- xp %>% filter(Percent < 20) %>% group_by(Party) %>% count() %>%
  mutate(Measurement = myMeasures[1])
x2 <- xp %>% filter(Percent < 30) %>% group_by(Party) %>% count() %>%
  mutate(Measurement = myMeasures[2])
x3 <- xp %>% filter(Percent < 40) %>% group_by(Party) %>% count() %>%
  mutate(Measurement = myMeasures[3])
x4 <- xp %>% filter(Percent < 50) %>% group_by(Party) %>% count() %>%
