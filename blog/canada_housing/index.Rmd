...
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/blog/canada_housing)"
---
...
STATCAN Table: 34-10-0126-01 - Housing starts, under construction and completions

> - `r shiny::icon("globe")` [https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=3410012601](https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=3410012601){target="_blank"}
> - `r shiny::icon("save")` [3410012601_databaseLoadingData.csv](3410012601_databaseLoadingData.csv)
...
> - `r shiny::icon("save")` [1110019001_databaseLoadingData.csv](1110019001_databaseLoadingData.csv)
...
```{r}
# Prep data
myCaption <- "Data: STATCAN<br>derekmichaelwright.github.io/dblogr/blog/canada_housing"
...
d2 <- read.csv("3410029201_databaseLoadingData.csv") %>%
  ...
  #mutate(Type.of.building = mv(Type.of.building, myTs1, myTs2),
...
#
d4 <- read.csv("3410012601_databaseLoadingData.csv") %>%
  select(Year=REF_DATE, Area=GEO, Measurement=Housing.estimates, 
         Type=Type.of.unit, Unit=UOM, Value=VALUE)
#
myTypes <- c("Total dwelling type", "Single house", "Double house", 
             "Row house", "Apartment building", "Mobile house")
d5 <- read.csv("3610068801_databaseLoadingData.csv") %>% 
...
# Prep data
xx <- d2 %>% 
  filter(Measurement == "Number of permits", Type.of.building == "Total residential")
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  ...
  labs(title = "Residential Building Permits in Canada", 
       y = "x 1000", x = NULL, caption = myCaption)
...
gg_Permits <- function(myArea = "Canada") {
  # Prep data
  myTypes <- c("Total residential and non-residential", 
               "Total residential", "Total non-residential")
  x1 <- d2 %>% filter(Area == myArea, Type.of.building %in% myTypes,
                      Measurement == "Number of permits")
  x2 <- d2 %>% filter(Area == myArea, Type.of.building %in% myTypes,
                      Measurement == "Value of permits")
  # Plot
  ...
  mp2 <- ggplot(x2, aes(x = Date, y = Value / 1000000)) +
    ...
    labs(subtitle = "B) Value of Permits", y = "Million CAD$", x = NULL,
         caption = myCaption)
  #
  ggarrange(mp1, mp2, ncol = 1, align = "hv", heights = c(1,1.2))
}
```
...
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value)) +
  geom_area(fill = "darkgreen", alpha = 0.8, color = "black") +
...
```{r}
xx <- d4 %>% filter(Area =="Canada", Type == "Total units")
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Measurement)) +
  ...
  scale_color_manual(name = NULL, values = myColors_P) +
  theme_agData(legend.position = "bottom") +
  labs(title = "Canada", subtitle = "Housing Units", 
...
```{r}
xx <- d4 %>% 
  filter(Area =="Canada", Type %in% c("Single-detached","Multiples"))
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Type)) +
  geom_line(alpha = 0.7,size = 2) +
  expand_limits(y = 0) +
  scale_color_manual(name = NULL, values = myColors_P) +
  theme_agData(legend.position = "bottom") +
  labs(title = "Canada", subtitle = "Housing Units", 
       y = "Thousand Units", x = NULL, caption = myCaption)
ggsave("canada_housing_4_02.png", mp, width = 6, height = 4)
```
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  ...
  labs(title = "Unoccupied Housing Units in Canada", 
       y = "Thousand Units", x = NULL, caption = myCaption)
ggsave("canada_housing_5_01.png", mp, width = 8, height = 6)
```
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  ...
  labs(title = "Unoccupied Housing Units in Canada", 
       y = "Thousand Units", x = NULL, caption = myCaption)
ggsave("canada_housing_5_02.png", mp, width = 12, height = 8)
```
...
ggsave("canada_housing_5_03.png", mp, width = 6, height = 4)
```
...
mp <- ggplot(xx, aes(x = Year, y = Value, color = Unit.Type)) +
  ...
  labs(title = "Rent Prices", y = "Dollars",
       x = NULL, caption = myCaption)
ggsave("canada_housing_6_01.png", mp, width = 40, height = 25)
```
...
# Prep data
xx <- d6 %>% 
  filter(Year == 2024, Unit.Type == "Two bedroom units",
         Structure.Type == "Row and apartment structures of three units and over") %>%
  arrange(desc(Value)) %>%
  mutate(Area = factor(Area, levels = .$Area))
xx <- bind_rows(xx %>% slice_min(Value, n = 15) %>% mutate(Group = "Top 15"),
                xx %>% slice_max(Value, n = 15) %>% mutate(Group = "Bottom 15"))
# Plot
...
## Vancouver - Toronto - Saskatoon {.tabset .tabset-pills}
...
# Household Spending {.tabset .tabset-pills}

## Food vs Housing {.tabset .tabset-pills}
...
```{r}
# Prep Data
xx <- d7 %>% filter(Area == "Canada", Measurement != "Total expenditure")
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Measurement)) +
  ...
  geom_point() +
  geom_point(data = xx %>% filter(Year < 2018), pch = 1, color = "black") +
...
ggsave("canada_housing_7_01.png", mp, width = 6, height = 4)
```
...
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Measurement)) +
  ...
  geom_point() +
  geom_point(data = xx %>% filter(Year < 2018), pch = 1, color = "black") +
  facet_wrap(. ~ Area, ncol = 6, scales = "free_y") +
  scale_color_manual(name = NULL, values = c("darkgreen","steelblue","black")) +
  theme_agData(legend.position = "bottom") +
  labs(title = "Household Spending - Canada",
       y = "Thousand Dollars", x = NULL, caption = myCaption)
ggsave("canada_housing_7_02.png", mp, width = 14, height = 6)
```

---

## Percent of Total expenditures {.tabset .tabset-pills}
...
mp <- ggplot(xx, aes(x = Year, y = Value, color = Measurement)) +
  ...
  theme_agData(legend.position = "bottom") +
  labs(title = "Household Spending - Canada",
       y = "Percent of Total Expenditures", x = NULL, caption = myCaption)
ggsave("canada_housing_7_03.png", mp, width = 8, height = 4)
```
...
mp <- ggplot(xx, aes(x = Year, y = Value, color = Area)) +
  ...
  #geom_point() +
  facet_wrap(. ~ Measurement, ncol = 1, scale = "free_y") +
  scale_color_manual(name = NULL, values = agData_Colors) +
  theme_agData() +
  labs(title = "Household Spending - Canada",
       y = "Percent of Total Expenditures", x = NULL, caption = myCaption)
ggsave("canada_housing_7_04.png", mp, width = 8, height = 6)
```
...
mp <- ggplot(d8, aes(x = Date, y = Value)) +
  ...
  labs(title = "Interest Rates in Canada", y = "Percent", x = NULL,
       captiopn = "derekmichaelwright.github.io/dblogr/ | Data: BoC")
...
> Note: one of these is adjusted for inflation and the other isnt.

![](canada_housing_8_02.png)
...
x1 <- read.csv("1110019001_databaseLoadingData.csv") %>%
  ...
  filter(Group == "Economic families and persons not in an economic family",
         Measurement == "Median total income", Year >= 1986) %>%
...
#
d9 <- left_join(x2, x1, by = c("Year", "Area")) %>% 
...
mp <- ggplot(xx, aes(x = Date, y = Value, color = Measurement)) +
  ...
  labs(title = "Canada", subtitle = "Housing Price vs. Income",
       y = "Percent Change", x = NULL, caption = myCaption)
ggsave("canada_housing_8_02.png", mp, width = 6, height = 4)
```

![](canada_housing_8_02.gif)
...
anim_save("canada_housing_8_02.gif", mp, 
...
```{r}
# Prep data
xx <- read.csv("data_housing_immigrants.csv") %>% 
  filter(Measurement != "Median PFIR")
# Plot
mp <- ggplot(xx, aes(x = Persons, y = Value/1000, fill = Persons)) +
  geom_col(position = "dodge") +
  facet_grid(Measurement ~ Province, scales = "free_y") +
  theme_agData(axis.text.x = element_blank())+
  labs(y = "Thousand Dollars")
ggsave("canada_housing_8_03.png", mp, width = 6, height = 4)
```
