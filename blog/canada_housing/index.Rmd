...
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Measurement)) +
  ...
  scale_color_manual(name = NULL, values = c("darkgreen","steelblue", "darkblue")) +
...
```{r}
# Prep Data
xx <- d8 %>% filter(Measurement != "Total expenditure")
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Measurement)) +
  stat_smooth(data = xx %>% filter(Year < 2020), alpha = 0.8,
              geom = "line", method = "lm", se = F, fullrange = T) +
  geom_line(alpha = 0.8, size = 2) +
  scale_color_manual(name = NULL, values = c("darkgreen","steelblue")) +
  theme_agData(legend.position = "bottom") +
  labs(title = "Household Spending - Canada",
       y = "Thousand Dollars", x = NULL, caption = myCaption)
ggsave("canada_housing_6_02.png", mp, width = 8, height = 4)
```
