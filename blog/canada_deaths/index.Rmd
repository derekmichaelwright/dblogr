...
> - `r shiny::icon("globe")` https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=1310039401
...
# Total Deaths 2010-2023 {.tabset .tabset-pills}
...
# Total Deaths 2016-2023 {.tabset .tabset-pills}
...
```{r}
# Prep data
d5 <- readxl::read_xlsx("data_canada_deaths.xlsx", "Death Rate") %>%
...
xx <- d5 %>% filter(Trait == "Death rate (per 1,000)") %>%
...
```{r}
# Prep data
d6 <- read_xlsx("data_canada_deaths.xlsx", "Life Expectancy") %>%
  mutate(Area = factor(Area, levels = Area))
xx <- d6 %>% select(Area, `Life expectancy in 2019`,
...
```{r}
mp <- ggplot(d6, aes(x = Area, fill = Area,
