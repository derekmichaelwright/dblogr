...
ggsave("canada_gdp_16.png", mp, width = 8, height = 6)
```
...
ggsave("canada_gdp_17.png", mp, width = 8, height = 6)
```
...
# Prep data
myIs <- c("Energy sector [T016]", "Industrial production [T010]",
          "Goods-producing industries [T002]")
xx <- d1 %>% filter(Industry %in% myIs) %>% group_by(Year, Industry) %>%
  summarise(Value = sum(Value))
# Plot
ggplot(xx, aes(x = Year, y = Value, color = Industry)) +
  geom_line() +
  theme_agData()
```
