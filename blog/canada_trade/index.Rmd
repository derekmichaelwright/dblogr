...
```{r}
# Prep data
myCaption <- "Data: STATCAN<br>derekmichaelwright.github.io/dblogr/blog/canada_trade"
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  geom_col(aes(fill = Balance), color = "black", lwd = 0.05, alpha = 0.8) +
...
yy <- d2 %>% 
  ...
  arrange(desc(Value))
xx <- d2 %>% 
  filter(Trade != "Trade Balance", Year < 2026,
         !Trading.Partner %in% c("All countries","European Union")) %>%
  arrange(desc(Value)) %>% 
  mutate(Trading.Partner = factor(Trading.Partner, levels = yy$Trading.Partner),
         Value = ifelse(Trade == "Import", -Value, Value))
...
xx <- d2 %>% 
  filter(Trade == "Trade Balance") %>%
  mutate(Trading.Partner = mv(Trading.Partner,
           c("Türkiye","Russian Federation", "United States", "South Korea"), 
...
for(i in 1997:max(xx$Year)) {
  ...
  mapBubbles2(dF = xi, nameX = "Lon", nameY = "Lat", nameZSize = "Value",
              ...
              fillPalette = c("steelblue", "darkred"),
              #symbolSize = 2, 
...
xx <- d2 %>% 
  filter(Trade != "Trade Balance",
         Trading.Partner %in% c("All countries", "United States")) %>%
  spread(Trading.Partner, Value) %>%
  mutate(Percent = 100 * `United States` / `All countries`)
# Plot
mp <- ggplot(xx, aes(x = Year, y = Percent, color = Trade)) +
  geom_line(lwd = 1.5) +
  scale_color_manual(name = NULL, values = c("steelblue", "darkred")) +
  scale_y_continuous(limits = c(0,100), minor_breaks = seq(0, 100, by = 10)) +
  theme_agData(legend.position = "bottom") +
  labs(title = "Canada", subtitle = "US trade as a percent of total import/exports ", 
       y = "Percent", x = NULL, caption = myCaption)
ggsave("canada_trade_11.png", mp, width = 6, height = 4)
```
