---
title: "Canadian Trade - Imports & Exports"
...
`r shiny::icon("globe")` https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=1210001101
...
dd <- read.csv("1210001101_databaseLoadingData.csv") %>%
  select(Year=1, Area=GEO, Trade, Basis, Seasonal.adjustment,
...
xx <- dd %>% filter(Trade == "Trade Balance", Date == "2022-11-15") %>%
  arrange(desc(Value))
dd <- dd %>% 
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000)) +
  geom_bar(aes(fill = Balance), stat = "identity", 
           color = "black", lwd = 0.1, alpha = 0.8) +
...
mp <- ggplot(xx, aes(x = Date, y = Value)) +
  geom_bar(aes(fill = Balance), stat = "identity", alpha = 0.9) +
  stat_smooth(geom = "line", alpha = 0.7) +
  scale_fill_manual(name = NULL, values = c("steelblue", "darkred")) +
  ...
  labs(title = "Canada's Trade Balance With Other Countries", 
       x = NULL, y = "Million $", caption = myCaption)
...
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value, fill = Trade)) +
...
gg_Trade <- function(myArea = "India") {
  ...
  xx <- dd %>% 
    ...
    mutate(Value = ifelse(Trade == "Import", -Value, Value))
  # Plot
  mp1 <- ggplot(xx, aes(x = Date, y = Value, fill = Trade)) +
    geom_bar(stat = "identity", alpha = 0.9) +
    scale_fill_manual(name = NULL, values = c("steelblue", "darkred")) +
    scale_x_date(date_breaks = "10 years", date_labels = "%Y") +
    theme_agData(legend.position = "bottom") +
    labs(subtitle = paste("Canada's Trade Balance With", myArea), 
         x = NULL, y = "Million $", caption = "")
  #
  xx <- dd %>% 
    filter(Trading.Partner == myArea, Trade == "Trade Balance") %>%
    mutate(Balance = ifelse(Value > 0, "Net Export", "Net Import"))
