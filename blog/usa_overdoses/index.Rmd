...
`r shiny::icon("globe")` https://data.cdc.gov/NCHS/Early-Model-based-Provisional-Estimates-of-Drug-Ov/v2g4-wqg2
...
overdosePlot1 <- function(area = "United States") {
  ...
  xx <- dd %>% filter(Outcome == "Drug Overdose") %>% 
    filter(Area %in% area)
  yy <- xx %>% filter(Date < "2020-01-01")
  # Plot
  ggplot(xx, aes(x = Date, y = Rolling.4.Week.Mean)) +
    geom_bar(stat = "identity", size = 1, fill = "darkred", alpha = 0.8) +
    stat_smooth(data = yy, geom = "line", method = "lm", se = F, fullrange = T) +
    stat_smooth(data = yy, geom = "line", method = "lm", se = F, alpha = 0.7, size = 1.5) +
...
overdosePlot2 <- function(area = "United States") {
  ...
  xx <- dd %>% 
    ...
    filter(Area %in% area, Outcome == "Drug Overdose", Year < 2021) %>% 
    group_by(Area, Year) %>% 
  ...
  ggplot(xx, aes(x = Year, y = Value / 1000)) +
    geom_bar(aes(fill = YearGroup), stat = "identity", 
             color = "black") +
...
```{r}
# Bonus USA Age Pyramid

#![](usa_overdoses_4_01.png)

# Prep data
xx <- read.csv("data_usa_age_sex.csv") %>%
  mutate(Age = factor(Age, levels = unique(.$Age)))
# Plot
mp <- ggplot(xx, aes(x = Age), color = "black") +
  geom_bar(aes(y = -Female), stat = "identity", fill = "palevioletred3") +
  geom_bar(aes(y = Male), stat = "identity", fill = "steelblue") +
  coord_flip() +
  theme_agData() +
  labs(caption = "ww.dblogr.com/ or derekmichaelwright.github.io/dblogr/")
ggsave("usa_overdoses_4_01.png", mp, width = 6, height = 4)
```
