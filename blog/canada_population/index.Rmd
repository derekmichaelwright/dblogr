...
```{r}
# Prep data
myCaption <- "Data: STATCAN<br>derekmichaelwright.github.io/dblogr/blog/canada_population"
...
d1 <- read.csv("1710000901_databaseLoadingData.csv") %>%
  ...
  mutate(Area = gsub("Northwest Territories including", "NWT &", Area),
         ...
         AreaShort = mv(Area, myAreas, myAreasShort),
...
mp <- ggplot(xx, aes(x = Date, y = Value / 1000000)) +
  geom_area(fill = "darkgreen", alpha = 0.7, color = "black") +
...
mp <- ggplot(d1, aes(x = Date, y = Value / 1000000)) +
  geom_area(fill = "darkgreen", alpha = 0.9) +
...
gg_ExcessMales_plot <- function(myArea = "Saskatchewan") {
  ...
  xx <- d2 %>% 
    ...
    ungroup() %>%
    spread(Gender, Value) %>%
    mutate(Percent = 100 * (`Men+` - `Women+`) / (`Men+` + `Women+`))
  # Plot
  ggplot(xx, aes(y = Percent, x = Year)) + 
