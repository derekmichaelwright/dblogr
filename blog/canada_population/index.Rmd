...
d1 <- read.csv("1710000901_databaseLoadingData.csv") %>%
  ...
  mutate(Area = factor(Area, levels = agData_STATCAN_Region_Table$Area),
         AreaShort = plyr::mapvalues(Area, agData_STATCAN_Region_Table$Area,
                                     agData_STATCAN_Region_Table$Area_Short),
...
xx <- d1 %>% 
  ...
  mutate(Group = ifelse(Area %in% c("British Columbia", "Alberta", 
         ...
         Rate = NA)
for(i in 1:nrow(xx)) {
  priorV <- 1000 * xx$Value[xx$Year == xx$Year[i]-1 & xx$Area == xx$Area[i]]
  if(length(priorV)>0) { xx$Rate[i] <- (xx$Value[i] - priorV) / priorV }
}
# Plot
...
ggsave("canada_population_1_05.png", mp, width = 10, height = 4)
```
...
mp <- ggplot(xx, aes(x = Year, y = Value, color = Sex)) +
  ...
  labs(title = "Median Age In Canada", x = NULL,
       y = "Median Age", caption = myCaption)
ggsave("canada_population_1_07.png", mp, width = 10, height = 6)
```
...
gg_PopDem_plot <- function(area = "Saskatchewan", years = 2019 ) {
  # Prep data
  xx <- d2 %>% 
    filter(Area == area, Year %in% years, 
...
```{r}
mp <- gg_PopDem_plot(area = "Canada", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_PopDem_plot(area = "British Columbia", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_PopDem_plot(area = "Alberta", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_PopDem_plot(area = "Saskatchewan", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_PopDem_plot(area = "Manitoba", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_PopDem_plot(area = "Ontario", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_PopDem_plot(area = "Quebec", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_PopDem_plot(area = "Yukon", years = c(1971, 1995, 2021))
...
gg_PopDem_anim <- function(area = "Saskatchewan") {
  # Prep data
  xx <- d2 %>% 
    filter(Area %in% area, Age != "Median age", Sex != "Both sexes")
  yy <- xx %>% spread(Sex, Value) %>% 
    mutate(Value = Females - Males,
           Sex = ifelse(Value < 0, "Males", "Females"))
  xx <- xx %>% 
...
gg_SexRatio_plot <- function(area = "Saskatchewan", years = 2019) {
  ...
  ggplot(xx, aes(y = Value / 1000, x = Age, fill = Group)) + 
    ...
    facet_grid(. ~ Year) +
    theme_agData(legend.position = "bottom") +
    labs(title = paste("Sex Ratio in", area), subtitle = "Females - Males", 
...
```{r}
mp <- gg_SexRatio_plot(area = "Canada", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_SexRatio_plot(area = "British Columbia", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_SexRatio_plot(area = "Alberta", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_SexRatio_plot(area = "Saskatchewan", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_SexRatio_plot(area = "Manitoba", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_SexRatio_plot(area = "Ontario", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_SexRatio_plot(area = "Quebec", years = c(1971, 1995, 2021))
...
```{r}
mp <- gg_SexRatio_plot(area = "Yukon", years = c(1971, 1995, 2021))
...
mp <- ggplot(xx, aes(x = Area, y = Percent, fill = Sex)) +
  ...
  theme_agData(legend.position = "bottom",
               axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Canada - Percent Of Population Under 65", x = NULL)
ggsave("canada_population_5_03.png", mp, width = 6, height = 4)
```
