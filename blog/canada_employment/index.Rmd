...
d2 <- d1 %>% 
  ...
  group_by(Area, Year, Measurement, Gender, Age) %>%
  summarise(Value = sum(Value)) %>% ungroup() %>%
  group_by(Area, Year, Gender, Age) %>% mutate(Total = sum(Value)) %>% ungroup() %>%
...
# Prep data
xx <- d1 %>% 
  filter(Area == "Canada", Gender == "Total - Gender", Age == "15 years and over",
...
xx <- d1 %>%
  filter(Area == "Canada", Gender != "Total - Gender", Age == "15 years and over",
         Measurement == "Employment rate")
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value, color = Gender)) +
...
xx <- d1 %>%
  filter(Area == "Canada", Gender != "Total - Gender", Age == "15 years and over",
         Measurement == "Employment rate") %>% 
  mutate(Year = as.numeric(substr(Date, 1, 4))) %>%
  group_by(Year, Gender, Unit) %>%
  summarise(Value = mean(Value))
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value, fill = Gender)) +
  geom_col(position = "dodge", alpha = 0.8) +
  scale_fill_manual(name = NULL, values = myCs_Gender) +
  theme_agData(legend.position = "bottom") +
  labs(title = "Canadian Employment Rate", x = NULL,
       y = "Percent", caption = myCaption)
...
xx <- d1 %>% 
  filter(Area == "Canada", Measurement == "Employment rate", 
         Age == "15 years and over", Gender != "Total - Gender", Date > "2010-01-01")
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value, color = Gender)) +
...
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000, color = Gender)) +
...
yy <- d1 %>% 
  filter(Area == "Canada", Date == "2020-02-15", Age == "15 years and over",
         Gender == "Total - Gender", Measurement %in% myMeasures)#, Measurement == "Employment")
...
# Prep data
xx <- d1 %>% 
  filter(Area != "Canada", Gender != "Total - Gender", Age == "25 to 54 years",
         Measurement == "Participation rate")
# Plot
mp <- ggplot(xx, aes(x = Date, y = Value / 1000, color = Gender)) +
  geom_line(size = 0.5, alpha = 0.8) +
