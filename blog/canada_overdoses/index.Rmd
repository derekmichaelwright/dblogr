...
`r shiny::icon("globe")` https://health-infobase.canada.ca/src/doc/SRHD/Update_Deaths_2023-06.pdf
...
`r shiny::icon("globe")` https://healthanalytics.alberta.ca/SASVisualAnalytics/?reportUri=%2Freports%2Freports%2F1bbb695d-14b1-4346-b66e-d401a40f53e6&sectionIndex=0&sso_guest=true&reportViewOnly=true&reportContextBar=false&sas-welcome=false
...
mp <- ggplot(bc2, aes(x = Date, y = Deaths)) + 
  ...
  labs(title = "Drug Overdoses in British Columbia", x = NULL,
       caption = myCaptionBC)
ggsave("british_columbia_overdoses_04.png", mp, width = 7, height = 4)
```
...
mp <- ggplot(bc3, aes(x = Year, y = Deaths, fill = Sex)) +
  ...
  labs(title = "Drug Overdoses in British Columbia", x = NULL,
       caption = myCaptionBC)
ggsave("british_columbia_overdoses_06.png", mp, width = 6, height = 4)
```
...
mp <- ggplot(xx, aes(x = `Age Group`, y = Deaths)) +
  ...
  labs(title = "Drug Overdoses in British Columbia", x = NULL,
       caption = myCaptionBC)
ggsave("british_columbia_overdoses_07.png", mp, width = 9, height = 4)
```
...
![](british_columbia_overdoses_09.png)
...
ggsave("british_columbia_overdoses_09.png", mp, width = 12, height = 4)
```
