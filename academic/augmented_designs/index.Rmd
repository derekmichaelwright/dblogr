...
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/academic/augmented_designs)"
---
...
myCaption <- "derekmichaelwright.github.io/dblogr/academic/augmented_designs"
```
...
dd <- read.csv("data_augmented_designs_1.csv") %>%
  mutate(Color = ifelse(Genotype %in% genotypes[1:4], Genotype, "Treatment"),
         Color = mv(Color, genotypes[1:4], checks),
...
First lets take a look at the field trial design.
...
dd <- read.csv("data_augmented_designs_1.csv") %>%
  mutate(Type = ifelse(Genotype %in% genotypes[1:4], Genotype, "Treatment"),
         Type = mv(Type, genotypes[1:4], checks),
...
Now lets plots all of the data to illustrate the block effect.
...
By runnaing an ANOVA on the replicated check varieties, we can get an estimate of the block effects and adjust our data accordingly.
...
# Print Coefficients
coef(fit)
# Calculate the final Block effect
# 
...
dd <- dd %>% 
  mutate(BlockEffect = mv(Block, blocks, blockeffects),
...
dd <- read.csv("data_augmented_designs_2.csv") %>% 
  ...
  mutate(Type = ifelse(MainCheck == 1, "Check1", "Treatment"),
         ...
         Block = mv(Block, unique(Block), 1:49))
```
