...
mp <- ggpairs(iris, columns = 1:4, aes(color = Species)) +
  ...
  theme_agData() +
  labs(title = "Iris Data Set", caption = myCaption1)
...
mp <- ggplot(xx, aes(x = Dim.1, y = Dim.2)) +
  geom_point(aes(color = Cluster, pch = Species), size = 2, alpha = 0.8) +
...
# Plot
mp <- mp +
  ggforce::geom_mark_hull(aes(fill = Cluster), expand = unit(3, "mm")) +
