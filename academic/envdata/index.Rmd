...
The purpose of this document is to provide a few examples of how to manipulate and plot the environmental data. The data for this vignette comes from the `EnvRtype` and `geosphere` R packages and lentil field trials in the AGILE project.

---

# Data

> - `r shiny::icon("r-studio")` https://github.com/allogamous/EnvRtype

Data sets can be downloaded from [**knowpulse.usask.ca/**](http://knowpulse.usask.ca/portal/) or my `r shiny::icon("github")` [**github**](https://github.com/derekmichaelwright/dblogr/tree/master/content/academic/envdata).

> - `r shiny::icon("save")` [environment_data.tar](https://github.com/derekmichaelwright/dblogr/raw/main/academic/envdata/environment_data.tar)
...
write.csv(xx, "AGILE_Expts.csv", row.names = F)
```
...
```{r}
# Prep data from 
xx <- read.csv("AGILE_Expts.csv")
DT::datatable(xx)
```

> - `r shiny::icon("save")` [My_EnvData.csv](My_EnvData.csv)
...
#
write.csv(ee, "My_EnvData.csv", row.names = F)
DT::datatable(ee)
```
...
mp <- ggplot(ee, aes(x = YYYYMMDD)) +
  ...
  theme_agData()
ggsave("envdata_1_01.png", mp, width = 8, height = 4)
```
...
ggsave("envdata_3_06.png", mp, width = 12, height = 23)
```
...
ggsave("envdata_3_08.png", mp, width = 12, height = 10)
```
...
ggsave("envdata_3_09.png", mp, width = 12, height = 5)
```
...
ggsave("envdata_3_10.png", mp, width = 12, height = 8)
```
...
ggsave("envdata_3_11.png", mp, width = 12, height = 4)
```
...
ggsave("envdata_3_12.png", mp, width = 12, height = 5)
```
...
ggsave("envdata_3_13.png", mp, width = 12, height = 4)
```
...
ggsave("envdata_3_14.png", mp, width = 12, height = 9)
```
...
ggsave("envdata_3_15.png", mp, width = 12, height = 5)
```
...
ggsave("envdata_3_17.png", mp, width = 12, height = 6)
```
...
ggsave("envdata_3_18.png", mp, width = 12, height = 12)
```
...
ggsave("envdata_3_19.png", mp, width = 12, height = 11)
```
...
mp <- ggplot(ee, aes(x = YYYYMMDD)) +
  ...
  facet_grid(. ~ env, scales = "free_x", space = "free_x") +
  theme_agData()
ggsave("my_expt_01.png", mp, width = 8, height = 4)
