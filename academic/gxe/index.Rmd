---
title: "GxE with Mixed Models"
subtitle: "A tutorial on how to run asses genotype by environment interactions in R"
date: "Last updated: `r Sys.Date()`"
author: "Derek Michael Wright [derekmichaelwright.github.io/dblogr](https://derekmichaelwright.github.io/dblogr/) <derek.wright@usask.ca> [github](https://github.com/derekmichaelwright/dblogr/tree/main/academic/gxe)"
---
...
# Prep data
myCaption <- "derekmichaelwright.github.io/dblogr/academic/gxe"
```
...
xx <- read.csv("https://raw.githubusercontent.com/derekmichaelwright/AGILE_LDP_Phenology/refs/heads/master/data/data_raw.csv") %>%
  mutate(ExptShort = mv(Expt, names_Expt, names_ExptShort),
         MacroEnv = mv(Expt, names_Expt, 
                      c(rep(names_MacroEnvs[1], 6), 
                        rep(names_MacroEnvs[2], 6), 
                        rep(names_MacroEnvs[3], 6)))) %>%
...
> - `r shiny::icon("save")` [data_gxe.csv](data_gxe.csv)
...
```{r}
gg_GxE_pie <- function(xx, ncol = 3) {
  myColors <- c("darkred", "purple4", "steelblue", "darkgreen")
  # Create the stacked bar plot
  mp <- ggplot(xx, aes(x = "", y = Proportion, fill = Component)) +
    geom_col(position = "stack", alpha = 0.7) +
    coord_polar("y", start = 0) +
    facet_wrap(. ~ Trait, ncol = ncol) +
    scale_fill_manual(values = myColors) +
    theme_agData_pie() +
...
> - `r shiny::icon("save")` [gxe_All.csv](gxe_All.csv)
...
# Plot
mp <- gg_GxE_bar(var_table)
ggsave("gxe_All_01.png", mp, width = 6, height = 4)
mp <- gg_GxE_pie(var_table)
ggsave("gxe_All_02.png", mp, width = 6, height = 4)
```
...
> - `r shiny::icon("save")` [gxe_Temperate.csv](gxe_Temperate.csv)

![](gxe_Temperate_01.png)

![](gxe_Temperate_02.png)
...
# Plot
mp <- gg_GxE_bar(var_table)
ggsave("gxe_Temperate_01.png", mp, width = 6, height = 4)
mp <- gg_GxE_pie(var_table)
ggsave("gxe_Temperate_02.png", mp, width = 6, height = 4)
```
...
> - `r shiny::icon("save")` [gxe_Mediterranean.csv](gxe_Mediterranean.csv)

![](gxe_Mediterranean_01.png)

![](gxe_Mediterranean_02.png)
...
# Plot
mp <- gg_GxE_bar(var_table)
ggsave("gxe_Mediterranean_01.png", mp, width = 6, height = 4)
mp <- gg_GxE_pie(var_table)
ggsave("gxe_Mediterranean_02.png", mp, width = 6, height = 4)
```
...
> - `r shiny::icon("save")` [gxe_SouthAsia.csv](gxe_SouthAsia.csv)

![](gxe_SouthAsia_01.png)

![](gxe_SouthAsia_02.png)
...
# Plot
mp <- gg_GxE_bar(var_table)
ggsave("gxe_SouthAsia_01.png", mp, width = 6, height = 4)
mp <- gg_GxE_pie(var_table)
ggsave("gxe_SouthAsia_02.png", mp, width = 6, height = 4)
```
