...
The Data used in this tutorial are sourced from [**knowpulse.usask.ca/**](http://knowpulse.usask.ca/portal/) and can be found in my [github repository](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/tree/master/data) for our [lentil phenology paper](https://nph.onlinelibrary.wiley.com/doi/10.1002/ppp3.10158). 

<font size="5"> `r shiny::icon("save")` [data_ldp.csv](data_ldp.csv) </font>

<font size="5"> `r shiny::icon("save")` [Tb and Pc (data_tb_pc.csv)](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/data_tb_pc.csv) </font>

<font size="5"> `r shiny::icon("save")` [data_env_sutherland_2017.csv](data_env_sutherland_2017.csv) </font>

<font size="5"> `r shiny::icon("save")` [data_env_morocco_2017.csv](data_env_morocco_2017.csv) </font>
...
eMo17 <- read.csv("data_env_morocco_2017.csv") %>%
  ...
  mutate(DateTime = as.POSIXct(paste(Date, Time), 
                               format = "%Y-%m-%d %H:%M:%OS"))
# Colors
myColors <- c("darkgreen", "steelblue", "darkblue", "darkorange",
              "darkred", "purple4")
...
We will be be using phenology data for a [Lentil Diversity Panel](https://dblogr.com/academic/lentil_diversity_panel/) grown in two different environments: Morocco and Canada.
