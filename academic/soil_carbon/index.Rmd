...
<font size="5"> `r shiny::icon("save")` [journal.pone.0260946.s002.docx](journal.pone.0260946.s002.docx) </font>

<font size="5"> `r shiny::icon("save")` [data_soil_carbon.xlsx](data_soil_carbon.xlsx) </font>
...
myColors <- c("darkblue", "darkgreen", "darkred")
# Plot
...
# Production Emissions vs Soil Capture
...
mp <- ggplot(d2, aes(x = Year, y = All, fill = Measurement)) +
  geom_bar(stat = "identity", position = "stack", 
           color = alpha("black",0.7), alpha = 0.7) +
...
# Prep data
xx <- d1 %>% filter(Measurement != "Net GHG Balance")
myColors <- c("darkblue", "steelblue", "black",
              "darkgoldenrod2", "cyan", "darkgreen")
# Plot
mp <- ggplot(xx, aes(x = Year, y = All, fill = Measurement)) +
  geom_bar(stat = "identity", position = "stack", 
...
x3 <- agData_STATCAN_Crops %>% 
  filter(Area == "Saskatchewan", Crop %in% myCrops,
         Year %in% 1985:2016, Measurement == "Production") %>%
  mutate(Crop = factor(Crop, levels = rev(myCrops)))
# Plot
mp1 <- ggplot(x3, aes(x = Year, y = Value / 1000000, fill = Crop)) +
...
x2 <- agData_STATCAN_Crops %>% 
  filter(Area == "Saskatchewan", Crop == "Canola",
...
mp <- ggplot(xx, aes(x = Year, y = All, fill = Measurement)) +
  ...
  scale_fill_manual(name = NULL, values = myColors[1:3]) +
...
```{r}
# Prep data
myColors <- c("darkgoldenrod3", "darkgoldenrod4", "grey70", "black", "grey30")
# Plot
