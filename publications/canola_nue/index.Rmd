...
downloadthis::download_link(
  link = "MastersThesis.pdf",
  button_label = "MastersThesis.pdf",
  button_type = "success",
  has_icon = TRUE,
  icon = "fa fa-file-pdf",
...
downloadthis::download_link(
  link = "data_canola_nue.xlsx",
  button_label = "data_canola_nue.xlsx",
  button_type = "success",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
gg_PlantMass <- function(nlevel, title) {
  x1 <- dd %>% 
    mutate(SeedMass  = ifelse(Genotype == "Pacific" & Nlevel == "N2", 
                              0, SeedMass),
           StrawMass = ifelse(Genotype == "Pacific" & Nlevel == "N2", 
                              0, StrawMass),
           StemMass  = ifelse(Genotype == "Pacific" & Nlevel == "N2", 
                              0, StemMass),
           RootMass  = ifelse(Genotype == "Pacific" & Nlevel == "N2", 
                              0, RootMass),
           RootScore = ifelse(Genotype == "Pacific" & Nlevel == "N2", 
                              0, RootScore))
  genoOrder <- x1 %>% 
...
mp <- ggarrange(mp1, mp2, ncol = 1) %>%
  annotate_figure(fig.lab.pos = "bottom.right",
                  fig.lab  = "\u00A9 www.dblogr.com/  |  Data: JLU")
...
mp <- ggplot(xx, aes(x = Genotype, y = RootMass, fill = Nlevel)) +
  ...
  theme_agData(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(caption = "\u00A9 www.dblogr.com/  |  Data: JLU")
...
ggN1N2 <- function(myTrait) {
  ...
  ggplot(xi, aes(x = Hybrid, y = Value)) +
    ...
    labs(y = NULL, x = NULL,
         caption = "\u00A9 www.dblogr.com/  |  Data: JLU")
}
```
