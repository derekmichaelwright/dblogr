...
downloadthis::download_link(
  link = "https://github.com/derekmichaelwright/dblogr/blob/master/content/dblogr/canada_2019_election/canada_2019_election_data.xlsx",
  button_label = "canada_2019_election_data.xlsx",
  button_type = "success",
  has_icon = TRUE,
  icon = "fa fa-save",
...
```{r echo = F, eval = F}
benfordsLaw <- function(party = "Cons", title = "Cons") {
  xx <- d2 %>% filter(Party_Short %in% party) %>%
    select(Area, District, Votes) %>%
    mutate(leadingDigit = substr(Votes, 1, 1))
  bl <- data.frame(d = 1:9) %>%
    mutate(p = log10(1 + 1/d),
           e = p * nrow(xx),
           o = NULL)
  for(i in 1:9) {
    bl$o[i] <- sum(xx$leadingDigit==i)
  }
  myP <- round(chisq.test(x = bl$o, p = bl$p)[[3]], 6)
  #
  ggplot() + 
    geom_bar(data = xx, aes(x = leadingDigit), 
             stat="count", alpha = 0.5, fill = "darkgreen") +
    geom_line(data = bl, aes(x = d, y = e)) + 
    geom_point(data = bl, aes(x = d, y = e)) +
    theme_agData() +
    labs(title = paste0(title, " (n = ", nrow(xx), ", p = ", myP, ")"))
}
mp <- benfordsLaw(party = c("Cons","Libs","NDP","Green","PPC"), "All")
ggsave("fraud/election_2019_00_fraud.png", mp, width = 6, height = 4)
```
