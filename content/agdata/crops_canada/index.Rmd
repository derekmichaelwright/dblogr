...
cropList <- function(measurement, years = c(1908, 1961, 2017)) {
  # Prep data
  xx <- agData_STATCAN_Crops %>% 
...
```{r}
# Prep data
myCrops <- cropList(measurement = "Production", years = c(1908, 1961, 2017))
xx <- agData_STATCAN_Crops %>% 
...
mp <- ggplot(xx, aes(x = Crop, y = PerPerson, fill = Crop)) + 
  ...
  labs(title = "Saskatchewan - Crop Production Per Person", y = "1000 Tonnes", x = NULL,
       caption = "\xa9 www.dblogr.com/  |  Data: STATCAN")
ggsave("crops_canada_06.png", mp, width = 6, height = 5)
```
...
ggsave("crops_canada_07.png", mp, width = 6, height = 4)
```
