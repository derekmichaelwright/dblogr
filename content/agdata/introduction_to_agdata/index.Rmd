...
- [agData_STATCAN_Population_AgeGender](#agdata_statcan_population_AgeGender)
- [agData_STATCAN_Population_Dynamics](#agdata_statcan_population_Dynamics)
...
```{r}
# Prep data for an example plot
xx <- xx %>% filter(Area == "World", Crop == "Cereals, Total", Measurement == "Yield")
# Plot
...
# agData_STATCAN_Population_AgeGender

```{r}
# Get Data
xx <- agData_STATCAN_Population_AgeGender %>% as_tibble()
xx
```
...
# Prep data for an example plot
xx <- xx %>% 
  filter(Area == "Saskatchewan", Sex %in% c("Males","Females"),
         Age == "0 to 17 years")
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000, color = Sex)) + 
  geom_line() + 
  theme_agData() +
  labs(title = "Population under 18 in Saskatchewan, Canada", 
       y = "Thousand People", x = NULL)
ggsave("Intro_agData_STATCAN_Population_AgeGender.png", mp, width = 6, height = 4)
```

![](Intro_agData_STATCAN_Population_AgeGender.png)

---

# agData_STATCAN_Population_Dynamics

```{r}
# Get Data
xx <- agData_STATCAN_Population_Dynamics %>% as_tibble()
xx
# List Measurements
xx %>% distinct(Measurement)
```
...
```{r}
# Prep data for an example plot
xx <- xx %>% filter(Area == "Saskatchewan", Measurement %in% c("Births","Deaths"))
# Plot
mp <- ggplot(xx, aes(x = Year, y = Value / 1000000, color = Measurement)) + 
  ...
  labs(title = "Population Dynamics in Saskatchewan, Canada", 
       y = "Million People", x = NULL)
ggsave("Intro_agData_STATCAN_Population_Dynamics.png", mp, width = 6, height = 4)
```

![](Intro_agData_STATCAN_Population_Dynamics.png)
