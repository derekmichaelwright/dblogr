...
xx <- agData_STATCAN_Crops %>% 
  filter(Area == "Saskatchewan", Year %in% c(2017, 1961, 1908),
...
anim_save("crops_saskatchewan_gifs_01.gif", mp, width = 800, height = 400)
```
...
# gganimate
  transition_states(Year)
mp <- animate(mp, nframes = 2*(max(xx$Year) - min(xx$Year)), fps = 10, end_pause = 20)
anim_save("crops_saskatchewan_gifs_02.gif", mp, width = 800, height = 400)
```
