...
links:
  ...
  - icon_pack: fas
    icon: file-pdf
    name: PDF
    url: https://github.com/derekmichaelwright/dblogr/raw/master/content/agdata/lentil/lentil_fao.pdf
---
...
**PDF**: [lentil_fao.pdf](https://github.com/derekmichaelwright/dblogr/raw/master/content/agdata/lentil/lentil_fao.pdf)
...
areas <- agData_FAO_Crops %>% addRegionInfo() %>%
  filter(Area %in% agData_FAO_Country_Table$Country,
         Year == max(Year), Crop == "Lentils", Measurement == "Production") %>%
  arrange(desc(Value)) %>% 
  slice(1:10) %>% 
