...
date: "`r Sys.Date()`"
author: "Derek Michael Wright <derek.wright@usask.ca> [www.dblogr.com/](https://dblogr.com/academic/augmented_designs)"
tags: [ "dblogr", "R" ]
draft: true
weight: 7
...
Augmented designs were developed (Federer, 1956) as a way of controlling error in plant breeding trials which often have many genotypes that need to be tested, and limited seed or resources to do proper replications of all material. Therefore, in order to control for the heterogeneity that exists within a field, a set of check cultivars are replicated in each *block*. The block effects and error estimated from the replicated checks, is then used to adjust the values of each new genotype being tested.

**Federer, W.** (1956) Augmented (or hoonuiaku) designs. *Hawaiian Planters Record*.

```{r echo = F}
downloadthis::download_link(
  link = "https://github.com/derekmichaelwright/dblogr/blob/master/content/academic/augmented_designs/ExampleData.xlsx",
  button_label = "ExampleData.xlsx",
  button_type = "success",
  has_icon = TRUE,
  icon = "fa fa-save",
...
myColors <- c("darkgreen", "darkred", "darkorange", "darkblue", "white")
xx <- read_xlsx("ExampleData.xlsx", "RCBD") %>%
  mutate(Color = ifelse(Genotype %in% genotypes, checks, "Treatment"))
# Plot
mp <- ggplot(xx, aes(x = "", y = Row)) + 
...
xx <- read_xlsx("ExampleData.xlsx", "RCBD") %>%
  mutate(Color = ifelse(Genotype %in% genotypes, checks, "Treatment"),
         Type = ifelse(Genotype %in% genotypes, "Check", "Treatment"))
# Plot data
mp <- ggplot(xx, aes(x = "", y = Row)) + 
  geom_tile(aes(fill = Color), color = "black", alpha = 0.75) + 
...
```{r}
# Prep data
myColors <- c("darkgreen", "darkred", "darkorange", "darkblue", "grey30")
xt <- xx %>% filter(Type != "Check")
xc <- xx %>% filter(Type == "Check")
# Plot
mp <- ggplot(xx, aes(x = Block, y = Yield)) + 
  geom_beeswarm(aes(color = Color, shape = Type), size = 3) +
  geom_text_repel(data = xt, aes(label = Genotype), nudge_x = 0.2) +
  scale_color_manual(name = NULL, values = myColors) +
  scale_shape_manual(values = c(16,15), guide = F) +
  guides(shape = F) + 
  ylim(c(70,96)) +
  theme_agData(legend.position = "bottom") +
...
# Plot
mp <- ggplot(xx, aes(x = Block, y = Yield)) + 
  geom_beeswarm(aes(color = Color, shape = Type, alpha = Type), size = 3) +
  geom_text_repel(data = xt, aes(label = Genotype), nudge_x = 0.2) +
  scale_color_manual(name = NULL, values = myColors) +
  scale_shape_manual(values = c(16,15), guide = F) +
...
# Plotting function
gg_Fieldplan <- function(xx, env) {
  xi <- xx %>% filter(Environment == env)
  mp <- ggplot(xi, aes(x = 1, y = SubPlotNum)) + 
    ...
    scale_y_reverse() + 
    labs(title = paste(env, "- Field plan")) +
    theme_agData(axis.text = element_blank(), axis.ticks = element_blank(),
                 strip.text.x = element_blank(), legend.position = "none")
  ggsave(paste0("Aug_02_01_", env, ".png"), mp, width = 20, height = 20)
}
gg_Fieldplan(xx = dd, env = "M2009") # S2009
gg_Fieldplan(xx = dd, env = "M2010") # S2010
gg_Fieldplan(xx = dd, env = "M2011") # S2011
gg_Fieldplan(xx = dd, env = "M2012") # S2012
```
...
```{r}
checkPlot <- function(env = "M2009", trait = "Yield") {
  xA <- dd %>% filter(Environment == env, MainCheck == 1)
  mp <- ggplot(xA, aes(x = Col, y = Row, fill = get(trait))) + 
    geom_tile() +
    geom_text(aes(label = get(trait))) +
...
![](Aug_02_02_M2009_Yield.png)

![](Aug_02_02_M2009_Iodine.png)

![](Aug_02_02_M2009_Linolenic.png)

![](Aug_02_02_M2009_Oil content.png)
...
$$R_i=\sum^{c}_{j=1}X_{ij(A)}/c-\bar{X}_A$$
...
$X_{ij(A)}$is the observed value for the control plot in the *ij*th block (CDC Bethune)
...
- $b$ = regression coefficient of the mean of control subplots ()
...
- `RDTF`: Rate of progress towards flowering

```{r eval = F}
# Prep data
d19 <- read_xlsx("NAM_2019_Sutherland.xlsx", "Measurements")
#
checktable <- d19 %>% filter(Generation == "Check", Block == 1) %>%
  select(Name, Check = Population) %>% arrange(Check) %>%
  mutate(NewName = paste(Check, Name, sep = "_"))
#
d19 <- d19 %>% filter(!is.na(Block)) %>%
  mutate(Name = plyr::mapvalues(Name, checktable$Name, checktable$Check))
#
aug_RCBD <- function(trait = "DTF", checks = checktable$Check) {
  #
  xx <- d19 %>% rename(Trait=trait) %>% filter(!is.na(Trait)) %>%
    mutate(Block = factor(Block), Name = factor(Name))
  out <- augmentedRCBD(block = xx$Block, treatment = xx$Name, y = xx$Trait, checks = checks, 
                       console = F, group = F)
  
  length(xx$Trait)
  length(unique(xx$Name))
  
  
  zz <- select(out[[2]], Name=Treatment, `Adjusted Means`)
  colnames(zz)[2] <- paste0(trait,"_Adj")
  xx <- left_join(xx, zz, by = "Name")
  #
  # Checks
  mp <- ggplot(zc, aes(x = Name, y = Trait, color = Name)) + 
